...
// Soloship injects its workflow rules unconditionally by design, so every
// generated rule is always-on. `globs: ""` + `alwaysApply: true` is Cursor's
// "apply to every request" combination; description-triggered rules would
// deliver the safety floor only sometimes, which is worse than its token cost.
