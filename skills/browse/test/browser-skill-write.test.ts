 ...
 * never touch the user's real ~/.soloship/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.soloship/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.soloship', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.soloship', 'browser-skills'),
    bundled: path.join(tmpRoot, 'soloship-install', 'browser-skills'),
  };
  // Synthetic tmp root keeps tests off the real ~/.soloship/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.soloship', '.tmp');
});
