 ...
 *   2. State file fallback: read `BROWSE_STATE_FILE` env or `<git-root>/.soloship/browse.json`
...
export interface BrowseClientOptions {
  ...
  timeoutMs?: number;
  /** Override state-file path. Default: BROWSE_STATE_FILE env or <git-root>/.soloship/browse.json. */
  stateFile?: string;
}
...
export function resolveBrowseAuth(opts: BrowseClientOptions = {}): ResolvedAuth {
  ...
  throw new Error(
    ...
    '(.soloship/browse.json must exist).'
  );
}
...
function defaultStateFile(): string | null {
  try {
    ...
    return path.join(base, '.soloship', 'browse.json');
  } catch {
    return path.join(process.cwd(), '.soloship', 'browse.json');
  }
}
