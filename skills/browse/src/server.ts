 ...
 *
 * State:
 *   State file: <project-root>/.soloship/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.soloship/browse-{console,network,dialog}.log
...
/**
 * Read ngrok authtoken from env var, ~/.soloship/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.soloship', 'ngrok.env');
...
async function shutdown(exitCode: number = 0) {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.soloship', 'chromium-profile');
...
async function start() {
  ...
  // except /connect.  Denials are logged to ~/.soloship/security/attempts.jsonl.
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          ...
          // would resolve to ~/.soloship/projects/../../etc/passwd/... — path
          ...
          // Fallback: built-in welcome page bundled next to the running
          // binary.  src/welcome.html is shipped as a sibling of the
          // compiled browse binary (or the source tree's src/ dir for
          // dev runs), so we resolve it relative to __dirname rather
          // than via an env-var-overridable install path.
          try {
            const builtinWelcome = path.join(__dirname, 'welcome.html');
            if (fs.existsSync(builtinWelcome)) return builtinWelcome;
            const srcWelcome = path.resolve(__dirname, '..', 'src', 'welcome.html');
            if (fs.existsSync(srcWelcome)) return srcWelcome;
          } catch {}
          return null;
        })();
        if (welcomePath) {
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        // 1) Resolve ngrok authtoken from env / .soloship / native config
