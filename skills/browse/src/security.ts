 ...
 *
 * Cross-process state lives at ~/.soloship/security/session-state.json
...
const SECURITY_DIR = path.join(os.homedir(), '.soloship', 'security');
 ...
 * ~/.soloship/security/device-salt (0600). Random per-device, prevents rainbow
...
// stripped: upstream telemetry-log subprocess integration removed for Soloship.
// Original code spawned an external helper binary to upstream attack attempts
// to a community telemetry tier. Soloship keeps only the local append-only
// audit trail at ~/.soloship/security/attempts.jsonl.

/**
 * Append an attempt to the local log. Never throws — logging failure should
 * not break the sidebar. Returns true if the local write succeeded.
 */
...
// per-tab file under ~/.soloship/security/decisions/, sidebar-agent polls
