...
2. **Identify Relevant Skills**:
   ...
   - Backend / server-side (e.g., Cloudflare Workers, Hono, edge runtimes) → official platform docs, framework guides, project conventions, and active repo examples
   - Frontend / Design → project design system, Figma/design artifacts when available, and active repo examples
   - TypeScript / React → official React and TypeScript docs, project conventions, and any available React best-practices guidance
   - Vanilla HTML/CSS/JS → web-platform (MDN) standards, accessibility guidance, and active repo examples
   - Platform scripting (e.g., Google Apps Script) → official platform reference docs, quotas/runtime constraints, and project conventions
...
**Tool Selection:** Use native file-search/glob (e.g., `Glob`), content-search (e.g., `Grep`), and file-read (e.g., `Read`) tools for repository exploration. Only use shell for commands with no native equivalent (e.g., inspecting an installed package's metadata), one command at a time.
