<!-- Vendored from compound-engineering v3.14.3 (Kieran Klaassen, MIT). See skills/vendored/ce/LICENSE. Scrubbed of Rails/Ruby-specific assumptions to be stack-neutral for Soloship's stacks (Cloudflare Workers, React/Vite/TS, vanilla JS, Apps Script). -->
...
| `wrangler.toml`, `wrangler.json`, `wrangler.jsonc` | Cloudflare Workers / Pages (bindings, routes, compatibility) |
| `vite.config.{js,ts}` | Vite build (React/Vue/Svelte SPA bundling) |
| `appsscript.json`, `*.gs`, `.clasp.json` | Google Apps Script |
| `index.html` with no build manifest | Vanilla HTML/CSS/JS static site (no build step) |
| `go.mod` | Go |
| `Cargo.toml` | Rust |
...
| Multiple sibling project directories each with their own `wrangler.toml` / `package.json` | Workspace-of-tools layout (one repo, several independently deployed projects) |
...
- **API surface:** If 0.1 found no web framework or server dependency, **and** the root listing shows no API-related directories or files (`routes/`, `api/`, `src/routes/`, `*.proto`, `openapi.yaml`, `swagger.json`): skip the API surface category. Report "None detected." Note: some runtimes serve HTTP with no visible framework dependency -- a Cloudflare Worker's `fetch` handler, a Go stdlib server, or an Apps Script `doGet`/`doPost` are all API surfaces with little or no manifest signal. Check the root listing and entry-point files for structural signals before skipping.
- **Data layer:** Evaluate independently from API surface -- a CLI, Worker, or cron job can have a data store without any HTTP layer. Skip only if 0.1 found no data-related dependency or binding (e.g., a D1/KV/R2 binding in `wrangler.toml`, prisma, drizzle, sequelize, typeorm, sqlalchemy, knex, diesel) **and** the root listing shows no data-related directories (`db/`, `prisma/`, `drizzle/`, `migrations/`, `models/`, `schema/`). Otherwise, check the data layer table below.
...
- If the root listing already showed deployment files (e.g., `wrangler.toml`, `fly.toml`, `vercel.json`): read them directly instead of globbing.
...
|----------------|-----------------|
| `wrangler.toml`, `wrangler.json`, `wrangler.jsonc` | Cloudflare Worker/Pages config: bindings (D1, KV, R2, Durable Objects), routes, cron triggers, compatibility date |
| `.clasp.json`, `appsscript.json` | Google Apps Script project + manifest (scopes, triggers, runtime) |
...
API surface (skip if no web framework, server entry point, or HTTP handler in 0.1):

| File / Pattern | What it reveals |
|----------------|-----------------|
| Worker/server entry point (`src/index.{ts,js}` exporting a `fetch` handler, Hono/itty-router app) | Cloudflare Worker HTTP routing |
| Route / handler directories (`routes/`, `src/routes/`, `src/api/`, `api/`) | HTTP routing patterns |
| `doGet`/`doPost` functions in `*.gs` | Apps Script web-app endpoints |
| `*.proto` | gRPC services |
| `*.graphql`, `*.gql` | GraphQL API |
| `openapi.yaml`, `swagger.json` | REST API specs |

Data layer (skip if no data store, ORM, binding, or migration tool in 0.1):

| File / Pattern | What it reveals |
|----------------|-----------------|
| D1/KV/R2/Durable Object bindings in `wrangler.toml` | Cloudflare-managed data stores (SQL, key-value, object storage, stateful objects) |
| Migration directories (`migrations/`, `drizzle/`, `prisma/migrations/`, `db/migrate/`, `alembic/`) | Database structure and evolution |
| ORM / model directories (`src/models/`, `models/`, `src/db/`) | Data model patterns |
| Schema files (`schema.sql`, `prisma/schema.prisma`, `drizzle/schema.ts`, `*.sql` DDL) | Data model definitions |
| Spreadsheet-backed data (`SpreadsheetApp` calls in `*.gs`, named ranges) | Apps Script data store via Google Sheets |
| Queue / event config (Cloudflare Queues, Redis, Kafka, SQS references) | Async patterns |

**0.3 Module Structure -- Internal Boundaries**

Scan top-level directories under `src/`, `lib/`, `app/`, `pkg/`, `internal/` to identify how the codebase is organized. For single-file projects (e.g., a vanilla static site, or a React app where the entire UI lives in one component file), note that the architecture is intentionally single-file and scan that file's internal sections instead. In monorepos where a specific service was scoped in 0.1b, scan that service's internal structure rather than the full repo.

**Using Phase 0 Findings**

If no dependency manifests or infrastructure files are found, note the absence briefly and proceed to the next phase -- the scan is a best-effort grounding step, not a gate. (A vanilla HTML/CSS/JS site with no build step legitimately has no manifest; absence is itself a finding.)
...
- Deployment model (static site, single Worker, multi-service, serverless, Apps Script, etc.)
...
5. **Codebase Pattern Search**
   ...
   - When mapping input/trust boundaries, grep for **all** input points rather than guessing a few -- the exact tokens vary by stack: request parsing and query/body access in a Worker (`request.json()`, `request.url`, `URLSearchParams`, route params from Hono/itty-router), form and event handlers in a React app (`onChange`, `onSubmit`, controlled-input state, `fetch`/loader calls), DOM reads in vanilla JS (`querySelector`, `addEventListener`, `URLSearchParams`, `dataset`), and event/parameter objects in Apps Script (`doGet(e)`/`doPost(e)`, `e.parameter`, `e.parameters`). Enumerate the relevant tokens for the detected stack, then grep each -- the technique is exhaustive enumeration of entry points, not reliance on any single framework's idiom.
...
- Deployment model (static site, single Worker, multi-service, serverless, Apps Script, etc.)
- API styles in use (REST, gRPC, GraphQL, Worker fetch handler, Apps Script web app, etc.)
- Data stores and async patterns
- Module organization style
