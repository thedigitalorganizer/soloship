...
types (e.g., /gs-qa, /ship). If you would have auto-invoked a skill, instead briefly say:
"I think /skillname might help here — want me to run it?" and wait for confirmation.
The user opted out of proactive behavior.

If `SKILL_PREFIX` is `"true"`, the user has namespaced skill names. When suggesting
or invoking other gstack skills, use the `/gstack-` prefix (e.g., `/gstack-qa` instead
of `/gs-qa`, `/gstack-ship` instead of `/ship`). Disk paths are unaffected — always use
...
> like suggesting /gs-qa when you say "does this work?" or /investigate when you hit
...
> This tells Claude to use specialized workflows (like /ship, /investigate, /gs-qa)
...
- Strategy, scope, "think bigger", "what should we build" → invoke /gs-plan-ceo-review
- Architecture, "does this design make sense" → invoke /gs-plan-eng-review
- Design system, brand, "how should this look" → invoke /design-consultation
- Design review of a plan → invoke /gs-plan-design-review
- Developer experience of a plan → invoke /gs-plan-devex-review
- "Review everything", full review pipeline → invoke /gs-autoplan
- Bugs, errors, "why is this broken", "wtf", "this doesn't work" → invoke /investigate
- Test the site, find bugs, "does this work" → invoke /gs-qa (or /gs-qa-only for report only)
- Code review, check the diff, "look at my changes" → invoke /review
- Visual polish, design audit, "this looks off" → invoke /gs-design-review
...
- Save progress, "save my work" → invoke /gs-context-save
- Resume, restore, "where was I" → invoke /gs-context-restore
- Security audit, OWASP, "is this secure" → invoke /gs-cso
...
1. **D-numbering.** First question in a skill invocation is `D1`. Increment per
   ...
   (e.g., `/gs-plan-ceo-review` running `/gs-office-hours` inline) starts its own
...
**When `/gs-context-restore` runs,** it parses `[gstack-context]` blocks from WIP
...
or calling /gs-context-save to save progress and start fresh.
...
Design/DX Review) with all zeros and verdict "NO REVIEWS YET — run `/gs-autoplan`".
...
\`~/.claude/skills/gstack/gs-plan-devex-review/dx-hall-of-fame.md\`
...
SLUG=$(~/.claude/skills/gstack/gs-browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
Locate `gs-office-hours`'s SKILL.md — it's either vendored inside the Soloship plugin (use Glob pattern `**/soloship/**/skills/gs-office-hours/SKILL.md`) or installed as part of gstack at `~/.claude/skills/gstack/office-hours/SKILL.md` (note: gstack stores its copy without the `gs-` prefix). Read whichever exists first using the Read tool.
...
Load the "## Pass 1" section from `~/.claude/skills/gstack/gs-plan-devex-review/dx-hall-of-fame.md`
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 1" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 2" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 3" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 4" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 5" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 6" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 7" section from it.
...
Load reference: find `dx-hall-of-fame.md` (sibling of this SKILL.md — use Glob pattern `**/gs-plan-devex-review/dx-hall-of-fame.md` to locate it, fall back to `~/.claude/skills/gstack/plan-devex-review/dx-hall-of-fame.md` if Soloship isn\'t installed), then read the "## Pass 8" section from it.
...
**Source attribution:** If the most recent entry for a skill has a \`"via"\` field, append it to the status label in parentheses. Examples: `plan-eng-review` with `via:"autoplan"` shows as "CLEAR (PLAN via /gs-autoplan)". `review` with `via:"ship"` shows as "CLEAR (DIFF via /ship)". Entries without a `via` field show as "CLEAR (PLAN)" or "CLEAR (DIFF)" as before.
...
| Design Review | \`/gs-plan-design-review\` | UI/UX gaps | {runs} | {status} | {findings} |
| DX Review | \`/gs-plan-devex-review\` | Developer experience gaps | {runs} | {status} | {findings} |
\`\`\`
