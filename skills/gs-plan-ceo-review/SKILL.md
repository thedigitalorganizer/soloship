...
```
If a design doc exists (from `/gs-office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.
...
from a prior CEO review session that paused so the user could run `/gs-office-hours`. Use it
...
sure," or is clearly exploring rather than reviewing — offer `/gs-office-hours`:
