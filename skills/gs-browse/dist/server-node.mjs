...
// src/telemetry.ts
function logTelemetry(_payload) {}

// src/cdp-allowlist.ts
...
var require_socksclient = __commonJS((exports) => {
  ...
  class SocksClient extends events_1.EventEmitter {
    ...
    static createConnectionChain(options, callback) {
      return new Promise((resolve7, reject) => __awaiter(this, undefined, undefined, function* () {
        try {
...
class BrowserManager {
  ...
  async launch() {
    ...
    this.browser.on("disconnected", () => {
      ...
      console.error("[browse] Console/network logs flushed to .soloship/browse-*.log");
      process.exit(1);
    });
  ...
  async launchHeaded(authToken) {
    ...
    if (extensionPath) {
      ...
      if (authToken) {
        ...
        const stateHomeDir = path5.join(process.env.HOME || "/tmp", ".soloship");
        mkdirSecure(stateHomeDir);
        const authFile = path5.join(stateHomeDir, ".auth.json");
        try {
    ...
    const fs3 = __require("fs");
    const path4 = __require("path");
    const userDataDir = path4.join(process.env.HOME || "/tmp", ".soloship", "chromium-profile");
...
// src/meta-commands.ts
init_read_commands();
init_commands();

// src/domain-skill-commands.ts
...
function soloshipHome() {
  return path6.join(os5.homedir(), ".soloship");
}
function globalFile() {
  return path6.join(soloshipHome(), "global-domain-skills.jsonl");
}
function projectFile(slug) {
  return path6.join(soloshipHome(), "projects", slug, "learnings.jsonl");
}
...
function deriveSlug() {
  ...
  try {
    ...
    if (top)
      return path7.basename(top);
  } catch {}
  return path7.basename(process.cwd()) || "unknown";
}
...
function formatSavedOk(row, slug) {
  return [
    ...
    `Version: ${row.version}`,
    `Stored at: ~/.soloship/projects/${slug}/learnings.jsonl`,
    "",
...
` + "Action: $B domain-skill save to create one first.");
  ...
  const tmpFile = path8.join(os6.tmpdir(), `soloship-domain-skill-${process.pid}-${Date.now()}.md`);
...
async function handlePromoteToGlobal(args) {
  ...
  return [
    ...
    `Stored at: ~/.soloship/global-domain-skills.jsonl`,
    "",
...
function defaultTierPaths(opts = {}) {
  ...
  return {
    project: projectRoot ? path9.join(projectRoot, ".soloship", "browser-skills") : null,
    global: path9.join(home, ".soloship", "browser-skills"),
    bundled: path9.join(bundledRoot, "browser-skills")
  };
}
...
function detectBundledRoot() {
  try {
    ...
    if (exec && /\/browse\/dist\/browse$/.test(exec)) {
      return path9.resolve(path9.dirname(exec), "..", "..");
    }
  } catch {}
...
function listBrowserSkills(tiers) {
  ...
  for (const { tier, root } of order) {
    ...
    for (const entry of entries) {
      ...
      if (seen.has(entry))
        continue;
      const dir = path9.join(root, entry);
      let stat;
      try {
      ...
      if (!stat.isDirectory())
        continue;
      const skillFile = path9.join(dir, "SKILL.md");
...
function readBrowserSkill(name, tiers) {
  ...
  for (const { tier, root } of order) {
    if (!root)
      continue;
    const dir = path9.join(root, name);
    const skillFile = path9.join(dir, "SKILL.md");
    if (!fs9.existsSync(skillFile))
      continue;
    try {
...
function handleShow2(args, ctx) {
  ...
  return readFile(path10.join(skill.dir, "SKILL.md"));
}
function readFile(p) {
...
async function handleTest(args, ctx) {
  ...
  if (!skill)
    throw new Error(`Skill "${name}" not found.`);
  const testFile = path10.join(skill.dir, "script.test.ts");
...
async function spawnSkill(opts) {
  ...
  try {
    ...
    const scriptPath = path10.join(opts.skill.dir, "script.ts");
...
function resolveMinimalPath() {
  ...
  if (bunPath && bunPath.includes("/bun")) {
    const dir = path10.dirname(bunPath);
    return `${dir}:${fallback}`;
  }
  return fallback;
}
...
function resolveConfig(env = process.env) {
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    stateDir = path11.dirname(stateFile);
    projectDir = path11.dirname(stateDir);
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path11.join(projectDir, ".soloship");
    stateFile = path11.join(stateDir, "browse.json");
  }
  return {
    projectDir,
    stateDir,
    stateFile,
    consoleLog: path11.join(stateDir, "browse-console.log"),
    networkLog: path11.join(stateDir, "browse-network.log"),
    dialogLog: path11.join(stateDir, "browse-dialog.log"),
    auditLog: path11.join(stateDir, "browse-audit.jsonl")
  };
}
function ensureStateDir(config) {
  ...
  const gitignorePath = path11.join(config.projectDir, ".gitignore");
  try {
    const content = fs11.readFileSync(gitignorePath, "utf-8");
    if (!content.match(/^\.soloship\/?$/m)) {
      const separator = content.endsWith(`
`) ? "" : `
`;
      fs11.appendFileSync(gitignorePath, `${separator}.soloship/
`);
    }
  } catch (err) {
    if (err.code !== "ENOENT") {
      const logPath = path11.join(config.stateDir, "browse-server.log");
      try {
...
function readVersionHash(execPath = process.execPath) {
  try {
    const versionFile = path11.resolve(path11.dirname(execPath), ".version");
...
function parsePdfFromFile(payloadPath) {
  try {
    validateReadPath(path12.resolve(payloadPath));
  } catch {
...
`);
    ...
    case "inbox": {
      ...
      const inboxDir = path12.join(gitRoot, ".context", "sidebar-inbox");
      ...
      for (const file of files) {
        try {
          const data = JSON.parse(fs12.readFileSync(path12.join(inboxDir, file), "utf-8"));
          messages.push({
      ...
      if (args.includes("--clear")) {
        for (const file of files) {
          try {
            fs12.unlinkSync(path12.join(inboxDir, file));
          } catch (err) {
            if (err?.code !== "ENOENT")
              throw err;
          }
...
`);
    ...
    case "state": {
      ...
      const stateDir = path12.join(config.stateDir, "browse-states");
      mkdirSecure(stateDir);
      const statePath = path12.join(stateDir, `${name}.json`);
      if (action === "save") {
...
var SECURITY_DIR = path13.join(os8.homedir(), ".soloship", "security");
var ATTEMPTS_LOG = path13.join(SECURITY_DIR, "attempts.jsonl");
var SALT_FILE = path13.join(SECURITY_DIR, "device-salt");
var MAX_LOG_BYTES = 10 * 1024 * 1024;
var STATE_FILE = path13.join(SECURITY_DIR, "session-state.json");
...
var DECISIONS_DIR = path13.join(SECURITY_DIR, "decisions");
function getStatus() {
...
// src/tunnel-denial-log.ts
import { promises as fsp } from "fs";
import * as path14 from "path";
import * as os9 from "os";
var LOG_DIR = path14.join(os9.homedir(), ".soloship", "security");
var LOG_PATH = path14.join(LOG_DIR, "attempts.jsonl");
var RATE_CAP = 60;
var WINDOW_MS = 60000;
var writeTimestamps = [];
...
function resolveNgrokAuthtoken() {
  ...
  const ngrokEnvPath = path15.join(home, ".soloship", "ngrok.env");
  ...
  const ngrokConfigs = [
    path15.join(home, "Library", "Application Support", "ngrok", "ngrok.yml"),
    path15.join(home, ".config", "ngrok", "ngrok.yml"),
    path15.join(home, ".ngrok2", "ngrok.yml")
  ];
...
function readTerminalPort() {
  try {
    const f = path15.join(path15.dirname(config.stateFile), "terminal-port");
...
function readTerminalInternalToken() {
  try {
    const f = path15.join(path15.dirname(config.stateFile), "terminal-internal-token");
...
async function shutdown(exitCode = 0) {
  ...
  try {
    safeUnlinkQuiet(path15.join(path15.dirname(config.stateFile), "terminal-port"));
  } catch {}
  try {
    safeUnlinkQuiet(path15.join(path15.dirname(config.stateFile), "terminal-internal-token"));
  } catch {}
  try {
    detachSession();
  } catch (err) {
  ...
  const profileDir = path15.join(process.env.HOME || "/tmp", ".soloship", "chromium-profile");
  for (const lockFile of ["SingletonLock", "SingletonSocket", "SingletonCookie"]) {
    safeUnlinkQuiet(path15.join(profileDir, lockFile));
  }
  ...
  const makeFetchHandler = (surface) => async (req) => {
    ...
    if (url.pathname === "/welcome") {
      const welcomePath = (() => {
        ...
        const projectWelcome = `${homeDir}/.soloship/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
        if (fs17.existsSync(projectWelcome))
          return projectWelcome;
        try {
          const builtinWelcome = path15.join(__dirname, "welcome.html");
          if (fs17.existsSync(builtinWelcome))
            return builtinWelcome;
          const srcWelcome = path15.resolve(__dirname, "..", "src", "welcome.html");
          if (fs17.existsSync(srcWelcome))
            return srcWelcome;
        } catch {}
        return null;
      })();
    ...
    if (url.pathname === "/file" && req.method === "GET") {
      ...
      return new Response(Bun.file(filePath), {
        headers: {
          ...
          "Content-Disposition": `inline; filename="${path15.basename(filePath)}"`,
          "Cache-Control": "no-cache"
        }
      });
    }
...
`));
  ...
  const state = {
    ...
    serverPath: path15.resolve(__browseNodeSrcDir, "server.ts"),
  ...
  try {
    ...
    if (fs17.existsSync(stateDir)) {
      ...
      for (const file of fs17.readdirSync(stateDir)) {
        const filePath = path15.join(stateDir, file);
        const stat = fs17.statSync(filePath);
  ...
  if (process.env.BROWSE_TUNNEL === "1") {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error("[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.soloship/ngrok.env");
    } else {
...
start().catch((err) => {
  ...
  try {
    const errorLogPath = path15.join(config.stateDir, "browse-startup-error.log");
    mkdirSecure(config.stateDir);
