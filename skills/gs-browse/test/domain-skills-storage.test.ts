...
const TMP_HOME = path.join(os.tmpdir(), `soloship-test-${process.pid}-${Date.now()}`);
// Redirect ~/.soloship/ writes during tests by overriding HOME (and USERPROFILE
// for Windows). Soloship-native code resolves storage relative to os.homedir(),
// so writes land at TMP_HOME/.soloship/ for the duration of the test.
...
beforeEach(async () => {
  ...
  await fs.mkdir(path.join(TMP_HOME, '.soloship', 'projects', 'test-slug'), { recursive: true });
});
...
describe('domain-skills: scope shadowing (T4)', () => {
  ...
  it('global skill fires for project that has no override', async () => {
    const m = await freshImport();
    await fs.mkdir(path.join(TMP_HOME, '.soloship', 'projects', 'other-slug'), { recursive: true });
...
describe('domain-skills: persistence (T5)', () => {
  ...
  it('tolerant parser drops partial trailing line on read', async () => {
    ...
    const file = path.join(TMP_HOME, '.soloship', 'projects', 'test-slug', 'learnings.jsonl');
