...
describe('logAttempt', () => {
  test('writes attempts.jsonl with correct shape', () => {
    ...
    const logPath = path.join(os.homedir(), '.soloship', 'security', 'attempts.jsonl');
...
// Bash-binary resolution + telemetry-spawn tests removed alongside the
// upstream telemetry-log subprocess integration (Soloship keeps only the
// local audit trail at ~/.soloship/security/attempts.jsonl).
