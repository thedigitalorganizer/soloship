 ...
 *
 * Dataset cache: ~/.soloship/cache/browsesafe-bench-smoke/test-rows.json
 * (hermetic after first run — no HF network traffic on subsequent CI).
 *
 * Run: bun test browse/test/security-bench.test.ts
 * Run with fresh sample: rm -rf ~/.soloship/cache/browsesafe-bench-smoke/ && bun test ...
 */
...
const CACHE_DIR = path.join(os.homedir(), '.soloship', 'cache', 'browsesafe-bench-smoke');
