...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/soloship-cli-');
    const execPath = path.join(root, '.claude/skills/soloship/skills/gs-browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/soloship/skills/gs-browse/src/server.ts');
    // The contract under test is dist/browse → ../src/server.ts adjacency,
    // so the surrounding directory layout is incidental.
