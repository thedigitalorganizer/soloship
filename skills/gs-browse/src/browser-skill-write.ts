 ...
 *
 * /skillify stages a candidate skill into ~/.soloship/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.soloship/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.soloship', '.tmp');
