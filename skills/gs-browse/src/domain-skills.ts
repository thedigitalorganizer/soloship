 ...
 *
 * Scope:
 *   - per-project: ~/.soloship/projects/<slug>/learnings.jsonl
 *   - global:      ~/.soloship/global-domain-skills.jsonl
 *
...
function soloshipHome(): string {
  return path.join(os.homedir(), '.soloship');
}

function globalFile(): string {
  return path.join(soloshipHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(soloshipHome(), 'projects', slug, 'learnings.jsonl');
}
