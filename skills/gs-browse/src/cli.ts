 ...
 *
 * Flow:
 *   1. Read .soloship/browse.json for port + token
...
/** Check if ngrok is installed and authenticated (native config or soloship env). */
function isNgrokAvailable(): boolean {
  // Check soloship's own ngrok env
  const ngrokEnvPath = path.join(process.env.HOME || '/tmp', '.soloship', 'ngrok.env');
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/soloship/skills/gs-browse`;
      try {
