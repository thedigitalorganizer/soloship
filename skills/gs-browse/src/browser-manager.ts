...
export class BrowserManager {
  ...
  private findExtensionPath(): string | null {
    ...
    const candidates = [
      ...
      // Relative to this source file (dev mode: gs-browse/src/ -> ../extension)
      path.resolve(__dirname, '..', 'extension'),
      // Also try the legacy two-up location used in some build layouts.
      path.resolve(__dirname, '..', '..', 'extension'),
      // Global Soloship install (skill ships under ~/.claude/skills/soloship/skills/gs-browse/extension)
      path.join(process.env.HOME || '', '.claude', 'skills', 'soloship', 'skills', 'gs-browse', 'extension'),
      // Git repo root (detected via BROWSE_STATE_FILE location)
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || '';
        if (stateFile) {
          const repoRoot = path.resolve(path.dirname(stateFile), '..');
          return path.join(repoRoot, '.claude', 'skills', 'soloship', 'skills', 'gs-browse', 'extension');
        }
        return '';
      })(),
    ].filter(Boolean);
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    if (extensionPath) {
      ...
      // Write to ~/.soloship/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const stateHomeDir = path.join(process.env.HOME || '/tmp', '.soloship');
        mkdirSecure(stateHomeDir);
        const authFile = path.join(stateHomeDir, '.auth.json');
        try {
    ...
    const fs = require('fs');
    const path = require('path');
    const userDataDir = path.join(process.env.HOME || '/tmp', '.soloship', 'chromium-profile');
    ...
    try {
      ...
      if (fs.existsSync(chromePlist)) {
        ...
        const iconCandidates = [
          path.join(__dirname, '..', 'scripts', 'app', 'icon.icns'),             // gs-browse repo dev mode
          path.join(__dirname, '..', '..', 'scripts', 'app', 'icon.icns'),       // legacy two-up dev layout
          path.join(process.env.HOME || '', '.claude', 'skills', 'soloship', 'skills', 'gs-browse', 'scripts', 'app', 'icon.icns'), // global install
        ];
