 ...
 *
 * Compiled to skills/gs-browse/dist/find-browse (standalone binary, no bun runtime needed).
...
export function locateBinary(): string | null {
  ...
  if (root) {
    candidates.push(join(root, '.claude', 'skills', 'soloship', 'skills', 'gs-browse', 'dist', 'browse'));
    candidates.push(join(root, '.claude', 'skills', 'gs-browse', 'dist', 'browse'));
  }

  // 2. Global Soloship installs — Claude Code plugin and user skill paths
  candidates.push(join(home, '.claude', 'plugins', 'marketplaces', 'soloship', 'skills', 'gs-browse', 'dist', 'browse'));
  candidates.push(join(home, '.claude', 'skills', 'soloship', 'skills', 'gs-browse', 'dist', 'browse'));
  candidates.push(join(home, '.claude', 'skills', 'gs-browse', 'dist', 'browse'));
  ...
  try {
    ...
    if (existsSync(pluginsRoot)) {
      for (const dir of readdirSync(pluginsRoot)) {
        const direct = join(pluginsRoot, dir, 'soloship', 'skills', 'gs-browse', 'dist', 'browse');
        ...
        try {
          for (const sub of readdirSync(marketplaceDir)) {
            const candidate = join(marketplaceDir, sub, 'skills', 'gs-browse', 'dist', 'browse');
...
function main() {
  ...
  if (!bin) {
    process.stderr.write('ERROR: browse binary not found. Run scripts/build-soloship.sh inside the gs-browse skill dir.\n');
    process.exit(1);
  }
