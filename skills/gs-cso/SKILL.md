...
## User-invocable
When the user types `/gs-cso`, run this skill.

## Arguments
- `/gs-cso` — full daily audit (all phases, 8/10 confidence gate)
- `/gs-cso --comprehensive` — monthly deep scan (all phases, 2/10 bar — surfaces more)
- `/gs-cso --infra` — infrastructure-only (Phases 0-6, 12-14)
- `/gs-cso --code` — code-only (Phases 0-1, 7, 9-11, 12-14)
- `/gs-cso --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `/gs-cso --diff` — branch changes only (combinable with any above)
- `/gs-cso --supply-chain` — dependency audit only (Phases 0, 3, 12-14)
- `/gs-cso --owasp` — OWASP Top 10 only (Phases 0, 9, 12-14)
- `/gs-cso --scope auth` — focused audit on a specific domain
...
**Daily mode (default, `/gs-cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`/gs-cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
