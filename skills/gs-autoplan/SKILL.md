...
**For each of the 4 review skills (`gs-plan-ceo-review`, `gs-plan-design-review`, `gs-plan-eng-review`, `gs-plan-devex-review`):**
...
2. If not found, fall back to the gstack install (requires gstack to be installed separately):
   - `~/.claude/skills/gstack/plan-<role>-review/SKILL.md` (note: gstack's copies do NOT carry the `gs-` prefix — that prefix is Soloship's namespace for vendored files)

Read the first path that exists. If neither exists for a required review (eng review is always required; design + DX are conditional), abort with: "Cannot find <skill-name>. Install the Soloship plugin or gstack, or run the reviews individually via /<skill-name>."

Load only:
- `gs-plan-eng-review` (always)
- `gs-plan-design-review` (only if UI scope detected)
- `gs-plan-ceo-review` (always for strategy review)
- `gs-plan-devex-review` (only if DX scope detected)
