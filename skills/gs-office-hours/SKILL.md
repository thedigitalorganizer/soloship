...
## Soloship preamble (run after SETUP)
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.soloship/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.soloship/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.soloship/projects/`.
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/soloship-codex-oh-XXXXXXXX.txt)
```
...
D=""
[ -x "$HOME/.claude/skills/soloship-design/dist/design" ] && D="$HOME/.claude/skills/soloship-design/dist/design"
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr ' ' '-')
_DESIGN_DIR="$HOME/.soloship/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/soloship-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/soloship-sketch.png
```
...
(`/gs-plan-design-review`, `/gs-design-review`) to see what was originally envisioned.
...
```bash
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' >> "$HOME/.soloship/builder-profile.jsonl"
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.soloship/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.soloship/projects/{slug}/{user}-{branch}-design-{datetime}.md`.

After writing the design doc, tell the user:
**"Design doc saved to: {full path}. Other skills (/gs-plan-ceo-review, /gs-plan-eng-review) will find it automatically."**
...
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.soloship/analytics/spec-review.jsonl 2>/dev/null || true
```
...
**Builder Journey Summary** (session 5+): Auto-generate `~/.soloship/builder-journey.md`
...
```bash
open "$HOME/.soloship/builder-journey.md"
```
...
Auto-generate updated `~/.soloship/builder-journey.md` with narrative arc. Open it.
...
```bash
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' >> "$HOME/.soloship/builder-profile.jsonl"
```

2. Log the selection to analytics:
```bash
mkdir -p ~/.soloship/analytics
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> ~/.soloship/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
- **`/gs-plan-ceo-review`** for ambitious features (EXPANSION mode) — rethink the problem, find the 10-star product
- **`/gs-plan-eng-review`** for well-scoped implementation planning — lock in architecture, tests, edge cases
- **`/gs-plan-design-review`** for visual/UX design review

The design doc at `~/.soloship/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
