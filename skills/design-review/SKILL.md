...
[ -f .soloship/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.soloship/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.soloship/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.soloship/no-test-bootstrap` and re-run." Continue without tests.
...
**Find the Soloship designer (optional — enables target mockup generation):**
...
if [ -z "$B" ]; then
  for FOUND in "$HOME"/.claude/plugins/*/soloship*/skills/browse/dist/browse \
...
REPORT_DIR="$HOME/.soloship/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
**Local:** `.soloship/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`
...
```
Write to: `~/.soloship/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
```
~/.soloship/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the Soloship designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
mkdir -p "$HOME/.soloship/projects/$SLUG"
```
Write a one-line summary to `~/.soloship/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
