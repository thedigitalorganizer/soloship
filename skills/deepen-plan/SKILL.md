...
produces evidence; when ambiguous, treat it as a gate. This skill is the delegation-discipline exemption: unbounded fan-out is its product in both postures.
...
This command takes an existing plan (from `/soloship:plan`) and enhances each section with parallel research agents. Each major element gets its own dedicated research sub-agent to find:
...
<thinking>
Check for documented learnings from /soloship:learn. These are solved problems stored as markdown files. Spawn a sub-agent for each learning to check if it's relevant.
</thinking>

**LEARNINGS LOCATION - Check these exact folders:**

```
docs/solutions/           <-- PRIMARY: Project-level learnings (created by /soloship:learn)
├── performance-issues/
│   └── *.md
├── debugging-patterns/
│   └── *.md
...
2. **Learnings/Solutions sub-agents** - Relevant documented learnings from /soloship:learn
...
2. **Run `/soloship:review`** - Get feedback from reviewers on enhanced plan
3. **Start `/soloship:implement`** - Begin implementing this enhanced plan
...
**After (from /soloship:deepen-plan):**
```markdown
## Technical Approach
