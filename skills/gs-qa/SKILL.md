...
version: 2.0.0
description: |
  Soloship — Systematically QA test a web application and fix bugs found. Runs QA testing,
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /gs-qa-only.
...
| Mode | full | `--regression .soloship/qa-reports/baseline.json` |
| Output dir | `.soloship/qa-reports/` | `Output to /tmp/gs-qa` |
...
```bash
mkdir -p .soloship/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.soloship/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr ' ' '-')
   ls -t ~/.soloship/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .soloship/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.soloship/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`
...
```
Write to `~/.soloship/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
