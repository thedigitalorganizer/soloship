...
### The Soloship designer — YOUR PRIMARY TOOL

You have the **Soloship designer**, an AI mockup generator that creates real visual mockups
...
`references/component-inventory.md`). Concretely flag: "the plan proposes a
new {X} but COMPONENTS.md already lists {Y} with the same purpose."
...
Soloship designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the Soloship designer. This is how we
...
`~/.soloship/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The Soloship
...
- `mkdir -p ~/.soloship/projects/$SLUG/designs/...`
...
_DESIGN_DIR="$HOME/.soloship/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The Soloship designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/soloship-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + Soloship design methodology.
...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.soloship/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
- **B)** Run /ceo-review (only if fundamental product gaps found)
