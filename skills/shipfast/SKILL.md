...
6. **Tear down browser sessions before reporting Done** (per `browser-tooling-priority`): close Chrome MCP tabs you created (`tabs_close_mcp`), release credential grants (`release_credentials`), close built-in-browser pages. Leave the `/soloship:browse` daemon running — its logins are shared by design.
