...
6. **Tear down browser sessions before reporting Done** (per `browser-qa-gate`): close Claude in Chrome tabs you created (`tabs_close_mcp`), release credential grants (`release_credentials`), close built-in-browser pages. Leave the `/soloship:browse` daemon running — its logins are shared by design.
