...
Do not make any code changes and do not start implementation. Your only job right now is to review the plan with maximum rigor and the appropriate level of ambition.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.soloship/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
### 0C-bis. Implementation Alternatives (always runs)

Before selecting a mode (0F), produce 2-3 distinct implementation approaches — every plan must consider alternatives; this step always runs.
...
Stop here and ask via AskUserQuestion — one question per issue, not batched. Recommend an option and say why. Wait for the user's response to 0C-bis before proceeding to Step 0D or 0F. A "clearly winning approach" is still an approach decision and still needs explicit user approval before it lands in the plan.
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr ' ' '-')
mkdir -p "$HOME/.soloship/projects/$SLUG"/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.soloship/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.soloship/projects/$SLUG/ceo-plans/{old-plan}.md ~/.soloship/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.soloship/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
3. Append metrics:
```bash
mkdir -p ~/.soloship/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.soloship/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + Soloship,
...
Stop here and ask via AskUserQuestion — one question per issue, not batched. Recommend an option and say why. If this section turned up zero findings, state "No issues, moving on" and proceed. If the section has findings, call AskUserQuestion as a tool_use — a finding with an "obvious fix" is still a finding and still needs user approval before any change lands in the plan. Wait for the user's response before proceeding.
**Reminder: Do NOT make any code changes. Review only.**
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+Soloship: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr ' ' '-')
rm -f ~/.soloship/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.soloship/` (user config directory, not project files). The skill preamble
already writes to `~/.soloship/sessions/` and `~/.soloship/analytics/` — this is
...
- **Outside Voice (optional):** Independent plan review from a different AI model. Offered after all review sections complete in /ceo-review and /eng-review. Falls back to Claude subagent if Codex is unavailable. Never gates shipping.
...
| Eng Review | \`/eng-review\` | Architecture & tests (required) | {runs} | {status} | {findings} |
| Design Review | \`/plan-design-review\` | UI/UX gaps | {runs} | {status} | {findings} |
| DX Review | \`/devex-review\` | Developer experience gaps | {runs} | {status} | {findings} |
\`\`\`
...
- **A)** Run /eng-review next (required gate)
...
- **B)** Keep in `~/.soloship/projects/` only (local, personal reference)
- **C)** Skip
