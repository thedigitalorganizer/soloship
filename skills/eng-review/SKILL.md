...
Stop here. For each issue found in this section, call AskUserQuestion individually — one issue per call, never batched into a single AskUserQuestion. Present options, state your recommendation, and explain why. Only proceed to the next section after every issue in this section is resolved.
...
Write to `~/.soloship/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:

```markdown
# Test Plan
Generated by /eng-review on {date}
Branch: {branch}
Repo: {owner/repo}
...
**Mention /ceo-review if this is a significant product change and no CEO review exists** — this is a soft suggestion, not a push. CEO review is optional. Only mention it if the plan introduces new user-facing features, changes product direction, or expands scope substantially.
...
- **B)** Run /ceo-review (only if significant product change and no CEO review exists)
