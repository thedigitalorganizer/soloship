...
If the Skill tool cannot find `ce-deepen-plan` by that exact name, read `${CLAUDE_PLUGIN_ROOT}/skills/ce-deepen-plan/SKILL.md` directly (or fall back to `~/.claude/plugins/marketplaces/soloship/skills/ce-deepen-plan/SKILL.md`) and execute its instructions as a complete workflow.
