---
name: shipthorough
description: Full due diligence deploy pipeline. Review, coverage audit, registry update, CHANGELOG, plan cleanup, bisectable commits, PR, verification, deploy. Use after significant work when everything needs to be solid.
---

Invoke the `shipthorough` skill from the Soloship plugin. Use the Skill tool with skill name `shipthorough` and let it drive the workflow.

If the Skill tool cannot find `shipthorough` by that exact name, read `${CLAUDE_PLUGIN_ROOT}/skills/shipthorough/SKILL.md` directly (or fall back to `~/.claude/plugins/marketplaces/soloship/skills/shipthorough/SKILL.md`) and execute its instructions as a complete workflow.

The skill file is the source of truth. Do not paraphrase or improvise — open it and follow it.
