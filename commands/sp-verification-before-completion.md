...
If the Skill tool cannot find `sp-verification-before-completion` by that exact name, read `${CLAUDE_PLUGIN_ROOT}/skills/sp-verification-before-completion/SKILL.md` directly (or fall back to `~/.claude/plugins/marketplaces/soloship/skills/sp-verification-before-completion/SKILL.md`) and execute its instructions as a complete workflow.
