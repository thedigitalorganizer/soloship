...
Invoke the `context-restore` skill from the Soloship plugin. Use the Skill tool with skill name `context-restore` and let it drive the workflow.

If the Skill tool cannot find `context-restore` by that exact name, read `${CLAUDE_PLUGIN_ROOT}/skills/context-restore/SKILL.md` directly (or fall back to `~/.claude/plugins/marketplaces/soloship/skills/context-restore/SKILL.md`) and execute its instructions as a complete workflow.
