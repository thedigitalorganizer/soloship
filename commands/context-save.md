...
Invoke the `context-save` skill from the Soloship plugin. Use the Skill tool with skill name `context-save` and let it drive the workflow.

If the Skill tool cannot find `context-save` by that exact name, read `${CLAUDE_PLUGIN_ROOT}/skills/context-save/SKILL.md` directly (or fall back to `~/.claude/plugins/marketplaces/soloship/skills/context-save/SKILL.md`) and execute its instructions as a complete workflow.
